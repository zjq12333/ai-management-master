...
- Latest safe UI NSIS smoke: commit `29adee0`, installer `AI Strategist_1.0.0_x64-setup.exe`, SHA256 `3ABA0940CB9466A65433D805D5A9679FA4D8488B6101881C81EC1C359D565658`; installed bridge `status` and `runtime-status` passed, installed `AI-Strategist.exe` opened and closed successfully, and no real `launch` / `stop-runtime` / `repair` command was executed.
- Latest ordinary-user NSIS smoke: commit `ad13919`, installer `AI Strategist_1.0.0_x64-setup.exe`, SHA256 `E51CB6E8758A7ADF95416E14A8B6D9C424056BBF03546BCBA01BC8727F462278`; installed bridge `status` and `runtime-status` passed, WindowsApps Codex helper processes are visible to the restart target, installed `AI-Strategist.exe` opened and closed successfully, and no real `launch` / `stop-runtime` / `repair` command was executed.
- Latest simplified-launcher NSIS smoke: commit `67576ec`, installer `AI Strategist_1.0.0_x64-setup.exe`, SHA256 `969918200C09FD69D863EBE324BA0F0F689B560EF7184BFC778422B7966713BF`; formal desktop shortcut points to `%LOCALAPPDATA%\Programs\AI Strategist\AI-Strategist.exe`, installed bridge `status` and `runtime-status` passed, installed UI opens with the ordinary-user Codex launcher and closes without crash events.
