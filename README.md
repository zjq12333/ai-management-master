...
AI Strategist 是面向本地 Codex Desktop 的桌面启动与修复工具。当前主界面采用 Tauri 版本，核心入口包括 **启动与修复** 和 **模型管理**。

它当前的主线目标有三层：

- 把 Codex 的启动、检查、修复集中到一个稳定桌面界面里；
- 把 provider/API key/模型路由/fallback/token/logs/health 收敛到本地统一模型桶；
...
2. **本地模型桶启动**
   - Codex 只连接本地统一入口，例如 `http://127.0.0.1:17431`。
   - provider/API key/模型路由/fallback/token/logs/health 由 `模型管理` 统一治理。
   - 启动与修复只检查并使用模型桶，不复制 provider 配置。

3. **混合模式启动**
   - 保留官方登录态，同时使用本地模型桶。
   - 目标是插件可用，同时模型请求走统一中转通道。
   - 当前要求：模型通道来自 `模型管理`，启动页不维护第二套 provider 配置。
...
3. 如果使用本地模型桶 / Hybrid，先到 `模型管理` 配置 provider、路由、token 和 Relay
4. 选择启动方式：
   - `官方账号启动`
   - `本地模型桶启动`
   - `混合模式启动`
...
- 切换模型桶 provider 或路由后，历史聊天突然不见了
- 官方账号能看到聊天，但非官方通道看不到
- 想在启动 Codex 前先确认本地模型桶、聊天桶和插件状态
...
- `启动与修复` 页面
- 官方 / 本地模型桶 / 混合三种启动模式
- 模型管理里的 provider、路由、token、Relay 和请求日志
- 启动前状态检查
- 真实修复入口
- Codex 运行中拦截提示
- 运行结果卡片
...
这些模块不是当前主线，当前主线只要求 `启动与修复` 能稳定承接官方 / API / 混合登录模式和修复能力。
